...
export const HeroParallax = ({ products }: { products: Product[] }) => {
  ...
  const translateX = useTransform(scrollYProgress, [0, 1], [0, isMobile ? 200 : 1000]);
  const translateXReverse = useTransform(scrollYProgress, [0, 1], [0, isMobile ? -200 : -1000]);
  const rotateX = useTransform(scrollYProgress, [0, 0.2], [isMobile ? 5 : 15, 0]);
  const opacity = useTransform(scrollYProgress, [0, 0.2], [0.5, 1]);
  const rotateZ = useTransform(scrollYProgress, [0, 0.2], [isMobile ? 5 : 20, 0]);
  const translateY = useTransform(scrollYProgress, [0, 0.2], [isMobile ? -200 : -700, isMobile ? 100 : 500]);

  return (
    <div
      ref={ref}
      className={`relative flex h-[300vh] flex-col overflow-hidden bg-[linear-gradient(180deg,#ffffff_0%,#fff7f7_46%,#ffffff_100%)] py-40 antialiased ${isMobile ? "" : "[perspective:1000px] [transform-style:preserve-3d]"}`}
    >
      <Header />
      <motion.div
        style={{
...
export const ProductCard = ({
  ...
  return (
    <motion.div
      ...
      key={product.title}
      className="group/product relative h-96 w-[90vw] shrink-0 overflow-hidden rounded-lg border border-white/70 bg-slate-950 shadow-[0_24px_80px_rgba(15,23,42,0.18)] ring-1 ring-slate-950/5 sm:w-[30rem]"
    >
      <Image
